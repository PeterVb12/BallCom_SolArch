...
namespace BallCom.Warehouse.API.Messaging
{
    public class OrderCancelledEventConsumer : BackgroundService
    {
        ...
        private readonly string _queueName = "warehouse.order-cancelled";
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            _logger.LogInformation("[Warehouse Service] OrderCancelled consumer luistert op queue '{Queue}'.", _queueName);
        }
        ...
        private async Task TryConnectAsync(CancellationToken stoppingToken)
        {
            ...
            for (var attempt = 1; attempt <= 10 && !stoppingToken.IsCancellationRequested; attempt++)
            {
                ...
                {
                    _logger.LogWarning(ex, "[Warehouse Service] RabbitMQ nog niet bereikbaar (poging {Attempt}/10).", attempt);
        ...
        {
            if (ea.RoutingKey != nameof(OrderCancelledEvent))
            {
            ...
            try
            {
                ...
                var cancelled = JsonSerializer.Deserialize<OrderCancelledEvent>(json);
            ...
            {
                _logger.LogError(ex, "[Warehouse Service] Fout bij verwerken OrderCancelledEvent.");
        ...
        private void HandleOrderCancelled(OrderCancelledEvent cancelled)
        {
            ...
            var pickList = context.PickLists.FirstOrDefault(p => p.OrderId == cancelled.OrderId);
            if (pickList is null)
            {
                _logger.LogInformation("[Warehouse Service] Geen pick list voor geannuleerde order {OrderId}.", cancelled.OrderId);
                return;
            }

            if (pickList.Status == PickListStatus.Cancelled || pickList.Status == PickListStatus.ReadyForShipment)
            {
                return;
            }
            ...
            var statusChanged = new PickListStatusChangedEvent(
                pickList.Id, pickList.OrderId, pickList.Status, PickListStatus.Cancelled, occurredAt);
            ...
            _logger.LogInformation("[Warehouse Service] PickList voor order {OrderId} geannuleerd.", cancelled.OrderId);
        }
